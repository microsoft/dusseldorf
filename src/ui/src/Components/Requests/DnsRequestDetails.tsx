...
import { DnsRespData, DssldrfRequest } from '../../Helpers/Types';
...
export const DnsRequestDetails = ({ details }: IDnsRequestDetailsProps) => {
    ...
    interface respType {
        TTL: number,
        ResponseData: DnsRespData,
        ResponseType: string,
        ResponseName?: string
    }

    // helper function in case we get a string instead of an object
    ...
    const req: reqType = jsonParse(details.request) as reqType
    const resp: respType = jsonParse(details.response) as respType

    // This is populated if we can parse the DNSResponseData correctly, 
    const respData = resp.ResponseType == "TXT" ?
                ...
                resp.ResponseType == "CNAME" ?
                    ...
                    resp.ResponseType == "NS" ?
                        ...
                        resp.ResponseType == "SOA" ?
                            `${resp.ResponseData.mname ?? ""}  ${resp.ResponseData.rname ?? ""} [${resp.ResponseData.times?.join(" ") ?? ""}]`
                            :
    ...
    return (
        <div className='stack vstack-gap'>
            ...
            <div className="stack vstack">
                ...
                <InfoLabel info={<div className="stack vstack-gap">
