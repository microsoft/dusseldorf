...
This folder contains the core components that make up the private Dusseldorf platform. These components are designed to be extended and customized:
...
### Setup
In your Entra ID tenant, [create an application registration](https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/CreateApplicationBlade/quickStartType~/null/isMSAApp~/false) with `http://localhost:8080/ui/` set as a redirect URI and `Single-page Application (SPA)` selected as the platform. Note the client ID and tenant ID of the newly created application.
...
This API is implemented using the FastAPI framework and provides CRUD functionality for zones and rules through a MongoDB database.  
...
By default, this permissive DNS server responds with its own IP address(es) to a name resolution. This behavior can be changed by assigning rules to your zone. Currently, custom `A`, `AAAA`, `CNAME`, and `TXT` records are supported.

Check the documentation for the DNS listener [here](listener.dns/README.md).


### HTTP/HTTPS Listener
This network listener can be set up to listen on cleartext HTTP traffic, or on HTTPS traffic if valid certificates are available. For a cleartext setup, this listener accepts HTTP requests on port 80/tcp and responds with default or customized HTTP responses.
...
If you have an x.509 public/private keypair in a wildcard certificate (for example `*.contoso.com`), you can set up the HTTP listener to listen on HTTPS.
